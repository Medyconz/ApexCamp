...
function renderProductEditor(){if(!productEditor)return;productEditor.innerHTML=(siteConfig.merch_products||[]).map((product,index)=>`<article class="editor-row product-row" data-index="${index}"><label>Product name <input data-field="name" value="${escapeAttr(product.name)}" required></label><label>Price <input data-field="price" value="${escapeAttr(product.price)}" placeholder="8 KWD"></label><label>Image URL <input data-field="image_url" value="${escapeAttr(product.image_url)}" placeholder="https://..."></label><label>Buy link <input data-field="buy_url" value="${escapeAttr(product.buy_url)}" placeholder="https://..."></label><label class="wide-field">Description <textarea data-field="description" rows="3">${escapeHtml(product.description)}</textarea></label><label class="check-row"><input type="checkbox" data-field="active" ${product.active!==false?'checked':''}> Visible on merch page</label><button class="button secondary" type="button" data-remove-product="${index}">Remove</button></article>`).join('');productEditor.querySelectorAll('[data-remove-product]').forEach((button)=>button.addEventListener('click',()=>{siteConfig.merch_products.splice(Number(button.dataset.removeProduct),1);renderProductEditor()}))}
